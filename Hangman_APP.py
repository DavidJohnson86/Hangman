...
class Application(Tk):
    ...
    def __init__(self):
        ...
        self.topic = Label(
            self, text=u'Téma: ' + self.topic_and_word[0],
            font=10, padx=5, pady=5, bg="white")
        ...
        self.image_on_canvas = self.can.create_image(
    ...
    def game_begin(self):
        ...
        self.secret_line = Label(self, font=(
            "Helvetica", 25, "bold"), bg='white')
        self.secret_line.configure(text=self.get_variable(self.Line))
        # Set to StringVar the Missed Letters
        self.missed_text = self.set_to_stringvar(
            u'Tévesztett betűk:' + self.missed_letters)
        # Get the StringVar variable
        self.dynamic_missed_text = Label(self, font=15)
        self.dynamic_missed_text.configure(
           text=self.missed_text.get(), bg='white')
        self.dynamic_missed_text.pack()
        '''Pack the canvas here'''
        self.can.pack()
        self.secret_line.pack()
        '''http://stackoverflow.com/questions/2261191/
        how-can-i-put-2-buttons-next-to-each-other'''
        self.Letters = ['A', u'Á', 'B', 'C', 'D', 'E', u'É', 'F', 'G', 'H',
                        'I', u'Í', 'J', 'K', 'L', 'M', 'N', 'O', u'Ó', u'Ö',
                        u'Ő', 'P', 'Q', 'R', 'S', 'T', 'U',  u'Ú', u'Ü', u'Ű',
                        'V', 'W', 'X', 'Y', 'Z']
    ...
    def game_decide(self):
        # This part of code will be updated when user
        # pushes the next button
        self.update_idletasks()
        self.input_data = self.input.lower()
        self.linespprove = Hangman_BSW.safety_input(
            self.input_data, self.missed_letters)
        # If the input is incorrect
        if self.linespprove is not True:
            tkMessageBox.showwarning('HIBA', self.linespprove)
        # if the input is correct
        else:
            result = Hangman_BSW.check_if_guessed(
                self.input_data, self.seperate_word)
            # If missed a letter
            if result == []:
                self.counter += 1
                # Increment Number of missed words by and
                # a missed letters to the list
                self.missed_words += 1
                self.missed_letters += self.input_data
                # Update Text of missed letters
                self.missed_text = (
                    u'Tévesztett betuk:' + ' '.join(self.missed_letters))
                ...
                if self.counter == 7:
                    ...
                    self.messagebox_ask(u':S Sajnos nem nyert',
                    u'Sajnos ez most nem sikerült akarsz újat játszani ?')
            # If the user found a relevant letter
            else:
                # Show the letters what the user found instead of lines
                self.show_secret_word = ''.join(
                    Hangman_BSW.print_table(
                        self.show_line, result, self.input_data))
                # Refresh the line
                self.secret_line.configure(text=self.show_secret_word)
                # If the user Win
                if self.secret_word == self.show_secret_word:
                    self.messagebox_ask(u"GRATULÁLOK NYERTÉL!!!!",
                        u"Gratulálok nyertél! Szeretnél újra játszani?")
    ...
    def button_pointer(self, i, arrange):
        Button(arrange,text=self.Letters[i],command =lambda: 
               self.test(self.Letters[i]), width=3 ).pack(side =LEFT)
        return
